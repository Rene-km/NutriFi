...
import MediapipeView, { type LandmarkPayload } from "./MediapipeView";
...
export default function ExerciseScreenWeb() {
  ...
  useEffect(() => {
    ...
    const restoreSession = async () => {
      const savedSessionId = await AsyncStorage.getItem("activeSessionId");
      const savedExercise = await AsyncStorage.getItem("activeExerciseName");
      ...
      if (savedSessionId && savedExercise === exerciseName) {
        ...
        if (!error && data) {
          // existing restore state logic
        } else {
          await AsyncStorage.removeItem("activeSessionId");
          await AsyncStorage.removeItem("activeExerciseName");
        }
      } else if (savedExercise !== exerciseName) {
        // existing stale-session reset logic
        setInSession(false);
        setIsPaused(false);
        setSessionId(null);
  ...
  // FIXED: useCallback with empty deps so this function reference never changes.
  // Without this, every setState call inside onLandmark triggers a re-render,
  // which gives the camera view a new onLandmark reference; on web that
  // restarts the requestAnimationFrame loop and freezes landmarks.
  // All session state is read via refs (inSessionRef, isPausedRef) so the
  // empty dep array is safe — the refs always hold the latest values.
  ...
  }, []); // empty deps — safe because all live state is read via refs
  ...
  const handleStart = async () => {
    ...
    if (data?.id) {
      ...
      setElapsedSeconds(0);
      startTimer();
      await AsyncStorage.setItem("activeSessionId", data.id);
      await AsyncStorage.setItem("activeExerciseName", exerciseName ?? "");
    }
  };  
  ...
  return (
    <View style={styles.root}>
      <View style={styles.cameraLayer} pointerEvents="box-none">
        <MediapipeView width={windowWidth} height={windowHeight} onLandmark={onLandmark} />
      </View>
